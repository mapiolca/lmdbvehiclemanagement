...
if ($canCreate) {
	...
	print '</table></div><div class="center"><button type="submit" class="button" name="action" value="save_certificate">'.$langs->trans('SaveInsuranceCertificateDraft').'</button> <button type="submit" class="button button-save" name="action" value="submit_certificate">'.$langs->trans('SubmitForReview').'</button></div></form>';
}
