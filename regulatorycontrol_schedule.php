...
$entityScope = getEntity('lmdbvehicle'); $allowedEntityIds = array_values(array_filter(array_map('intval', explode(',', $entityScope)))); $entityOptions = lmdbVehicleManagementGetEntityOptions('lmdbvehicle'); $showEntityColumn = !empty($entityOptions); if ($showEntityColumn) $arrayfields['req.entity'] = array('label' => 'Environment', 'checked' => 1, 'enabled' => 1, 'position' => 90); else $searchEntities = array();
...
$visibleStatus = LmdbVehicleSharing::requirementStatusSql($db);
$where = ' WHERE '.LmdbVehicleSharing::sql($db, 'lmdbvehicle', 'v');
...
if ($searchVehicle > 0) $where .= ' AND v.rowid = '.$searchVehicle;
...
if ($searchStatus !== '') $where .= " AND (".$visibleStatus.") = '".$db->escape($searchStatus)."'";
...
if ($showEntityColumn && !empty($searchEntities)) { $filtered = array_values(array_intersect($allowedEntityIds, array_map('intval', $searchEntities))); $where .= !empty($filtered) ? ' AND v.entity IN ('.implode(',', $filtered).')' : ' AND 1 = 0'; }
$from = ' FROM '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_vehicle AS v LEFT JOIN '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_control_requirement AS req ON v.rowid = req.fk_vehicle AND v.entity = req.entity AND req.active = 1 LEFT JOIN '.MAIN_DB_PREFIX.'c_lmdbvehiclemanagement_asset_type AS at ON at.rowid = v.fk_asset_type LEFT JOIN '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_regulatory_rule AS r ON r.rowid = req.fk_rule AND r.entity = req.entity LEFT JOIN '.MAIN_DB_PREFIX.'c_lmdbvehiclemanagement_control_type AS ct ON ct.rowid = r.fk_control_type AND ct.entity = r.entity LEFT JOIN '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_regulatory_control AS c ON c.rowid = req.fk_last_control AND '.LmdbVehicleSharing::sql($db, 'lmdbvehicleregulatorycontrol', 'c').' LEFT JOIN '.MAIN_DB_PREFIX.'societe AS s ON s.rowid = c.fk_soc_provider';
...
$sql = 'SELECT req.*, v.rowid AS fk_vehicle, v.entity AS entity, '.$visibleStatus.' AS visible_status, v.ref AS vehicle_ref, v.registration_number, v.label AS vehicle_label, v.regulatory_territory, at.label AS asset_type_label, r.label AS rule_label, ct.label AS control_type_label, c.ref AS control_ref, c.fk_soc_provider, s.nom AS provider_name,';
$sql .= ' (SELECT GROUP_CONCAT(p.label ORDER BY p.position SEPARATOR \'||\') FROM '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_vehicle_regulatory_profile AS vp INNER JOIN '.MAIN_DB_PREFIX.'c_lmdbvehiclemanagement_regulatory_profile AS p ON p.rowid = vp.fk_profile WHERE vp.entity = req.entity AND vp.fk_vehicle = req.fk_vehicle AND (vp.confirmed = 1 OR vp.origin = \'deduced\')) AS profile_labels';
$sql .= $from.$where.$db->order($sortfield === 'req.status' ? 'visible_status' : ($sortfield === 'req.entity' ? 'v.entity' : $sortfield), $sortorder).$db->plimit($limit + 1, $offset); $resql = $db->query($sql); if (!$resql) { dol_print_error($db); exit; } $num = $db->num_rows($resql);
...
$i = 0; while ($i < min($num, $limit) && is_object($row = $db->fetch_object($resql))) { $vehicle = new LmdbVehicle($db); $vehicle->id = (int) $row->fk_vehicle; $vehicle->ref = (string) $row->vehicle_ref; $vehicle->registration_number = (string) $row->registration_number; $vehicle->label = (string) $row->vehicle_label; $provider = new Societe($db); $provider->id = (int) $row->fk_soc_provider; $provider->name = (string) $row->provider_name; $statusTypes = array('incomplete' => 'status3', 'up_to_date' => 'status4', 'due_soon' => 'status1', 'overdue' => 'status8', 'recheck_required' => 'status6', 'non_compliant_blocking' => 'status8', 'derogation_active' => 'status1');
	...
	if (!empty($arrayfields['r.label']['checked'])) print '<td>'.dol_escape_htmltag($langs->trans(!empty($row->rowid) ? (string) $row->rule_label : 'VehicleToQualify')).'</td>';
	if (!empty($arrayfields['req.status']['checked'])) print '<td class="center">'.dolGetStatus(isset($statusOptions[$row->visible_status]) ? $statusOptions[$row->visible_status] : $langs->trans('Unknown'), '', '', $statusTypes[$row->visible_status] ?? 'status0', 5).'</td>';
