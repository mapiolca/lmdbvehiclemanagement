...
if (!isModEnabled('lmdbvehiclemanagement') || !isModEnabled('agenda') || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'read')) || !empty($user->socid)) accessforbidden();
...
$where = lmdbVehicleConsumptionAgendaWhere($object, 'a');
if ($searchLabel !== '') $where .= natural_search('a.label', $searchLabel);
$resCount = $db->query('SELECT COUNT(*) AS total FROM '.MAIN_DB_PREFIX.'actioncomm AS a'.$where);
$total = 0;
...
$sql = 'SELECT a.id, a.ref, a.datep, a.code, a.label, a.percent, a.fk_user_author, a.fk_user_action, u.login, u.firstname, u.lastname';
...
print dol_get_fiche_head($head, 'agenda', $langs->trans('ConsumptionEntry'), -1, $object->picto);
...
print '<div class="div-table-responsive-no-min"><table class="noborder centpercent liste"><tr class="liste_titre_filter"><td></td><td></td><td></td><td><input class="flat maxwidth300" name="search_label" value="'.dol_escape_htmltag($searchLabel).'"></td><td></td><td>'.$form->showFilterButtons().'</td></tr><tr class="liste_titre">';
...
print getTitleFieldOfList('Status', 0, $_SERVER['PHP_SELF'], 'a.percent', '', $param, 'class="center"', $sortfield, $sortorder, 'center ').'</tr>';
$rendered = 0;
while ($rendered < min($num, $limit) && is_object($row = $db->fetch_object($resql))) {
	$agendaObject = new ActionComm($db);
	$agendaObject->id = (int) $row->id; $agendaObject->ref = (string) $row->ref; $agendaObject->label = (string) $row->label;
	$agendaObject->datep = $db->jdate($row->datep); $agendaObject->percentage = (int) $row->percent; $agendaObject->authorid = (int) $row->fk_user_author;
	$agendaObject->userownerid = (int) $row->fk_user_action; $agendaObject->type_code = (string) $row->code;
	if (!$canReadAllAgenda) $agendaObject->userassigned[(int) $user->id] = array('id' => (int) $user->id, 'transparency' => 0);
	$owner = new User($db); $owner->id = (int) $row->fk_user_action; $owner->login = (string) $row->login; $owner->firstname = (string) $row->firstname; $owner->lastname = (string) $row->lastname; $owner->statut = 1;
	print '<tr class="oddeven"><td>'.$agendaObject->getNomUrl(1).'</td><td>'.dol_print_date($agendaObject->datep, 'dayhour').'</td><td>'.($owner->id > 0 ? $owner->getNomUrl(1) : '').'</td><td>'.dol_escape_htmltag((string) $row->code).'</td><td>'.dol_escape_htmltag((string) $row->label).'</td><td class="center">'.$agendaObject->LibStatut((int) $row->percent, 5).'</td></tr>';
	$rendered++;
}
