...
foreach ($groups as $key => $group) {
	...
	print '<div class="fichecenter"><div class="fichehalfleft">'.lmdbVehicleConsumptionRenderGraph($series, (string) $group['category'] === 'fuel' ? 'consumption_100' : 'quantity', $langs->trans((string) $group['category'] === 'fuel' ? 'AverageConsumption100' : 'Quantity').' - '.dol_escape_htmltag((string) $group['consumable_label']), 'global-'.$key.'-main').'</div>';
	print '<div class="fichehalfright">'.lmdbVehicleConsumptionRenderGraph($series, 'unit_price', $langs->trans('UnitPrice').' - '.dol_escape_htmltag((string) $group['consumable_label']), 'global-'.$key.'-price').'</div></div><div class="clearboth"></div>';
	if ((string) $group['category'] === 'fuel') {
		print '<div class="fichecenter"><div class="fichehalfleft">'.lmdbVehicleConsumptionRenderGraph($series, 'quantity', $langs->trans('Quantity').' - '.dol_escape_htmltag((string) $group['consumable_label']), 'global-'.$key.'-quantity').'</div>';
		print '<div class="fichehalfright">'.lmdbVehicleConsumptionRenderGraph($series, 'capacity_percent', $langs->trans('RecoveredCapacity').' - '.dol_escape_htmltag((string) $group['consumable_label']), 'global-'.$key.'-capacity').'</div></div><div class="clearboth"></div>';
	}
}
llxFooter();
$db->close();
