...
checkSharing($xpath->query('//*[@id="qx-sharing-open"]')->length === 1 && $xpath->query('//input[@name="token"]')->length === 1, 'Owner has one sharing action and one protected form');
...
ob_start(); include dirname(__DIR__).'/tpl/quartix_sharing.tpl.php'; $html = ob_get_clean();
checkSharing($html === '', 'Beneficiary administrators have no QUARTIX sharing form or action');
$conf->entity = 1;
