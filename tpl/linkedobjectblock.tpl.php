...
foreach ($linkedObjectBlock as $linkId => $invoice) {
	...
	if ((int) $object->entity === (int) $conf->entity && (int) $invoice->entity === (int) $conf->entity && (isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', $sourceRight, 'write')) && $user->hasRight('fournisseur', 'facture', 'creer')) {
