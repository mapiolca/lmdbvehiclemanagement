...
require_once DOL_DOCUMENT_ROOT.'/core/class/html.formprojets.class.php';
...
$odLocked = 0;
if ($id > 0 && !empty($object->fk_payment_various)) {
	$odLocked = $consumptionPayment->isLocked($object);
	if ($odLocked < 0) {
...
/** @param LmdbVehicleConsumption $target Target @param bool $financialLocked Preserve OD source fields @param bool $canSelectProject Project access @return void */
function lmdbConsumptionPopulateFromPost($target, $financialLocked = false, $canSelectProject = false)
{
	if (!$financialLocked) {
		...
		if ($canSelectProject) {
			...
			$target->fk_project = $projectId > 0 ? $projectId : null;
		} elseif (empty($target->id)) {
			$target->fk_project = null;
		}
	}
...
if (empty($reshook)) {
	if ($action === 'add' || $action === 'update') {
		...
		$canSelectProject = isModEnabled('project') && $user->hasRight('projet', 'lire');
		lmdbConsumptionPopulateFromPost($object, $action === 'update' && $odLocked > 0, $canSelectProject);
		$upload = isset($_FILES['receipt']) && is_array($_FILES['receipt']) ? $_FILES['receipt'] : array();
		$result = $action === 'add'
			? $consumptionPayment->createConsumption($object, $upload, GETPOSTINT('payment_mode_id'), $user)
			: $consumptionPayment->updateConsumption($object, $user);
		if ($result > 0) {
		...
		lmdbVehicleManagementSetObjectErrors($consumptionPayment->error !== '' ? $consumptionPayment : $object);
		$action = $action === 'add' ? 'create' : 'edit';
	} elseif ($action === 'confirm_delete' && $confirm === 'yes') {
		if (!$permissionDelete || $id <= 0) accessforbidden();
		$result = $consumptionPayment->deleteConsumption($object, $user);
		if ($result > 0) {
		...
		lmdbVehicleManagementSetObjectErrors($consumptionPayment->error !== '' ? $consumptionPayment : $object);
		$action = 'view';
	} elseif ($action === 'replace_receipt') {
		if (!$permissionWrite || $id <= 0 || empty($object->fk_payment_various)) accessforbidden();
		$upload = isset($_FILES['receipt']) && is_array($_FILES['receipt']) ? $_FILES['receipt'] : array();
		if ($consumptionPayment->replaceReceipt($object, $upload, $user) > 0) {
			setEventMessages($langs->trans('ConsumptionReceiptReplaced'), null, 'mesgs');
...
$canReadProject = isModEnabled('project') && $user->hasRight('projet', 'lire');
...
if ($action === 'create' || $action === 'edit') {
	...
	$financialLocked = $action === 'edit' && $odLocked > 0;
	$odFeatureEnabled = LmdbVehicleConsumptionPayment::isEnabled();
	if ($action === 'create' && $odFeatureEnabled) {
		$odConfigurationCheck = new LmdbVehicleConsumptionPayment($db);
		if ($odConfigurationCheck->validateConfiguration(
			getDolGlobalInt(LmdbVehicleConsumptionPayment::CONST_BANK_ACCOUNT),
			getDolGlobalInt(LmdbVehicleConsumptionPayment::CONST_PAYMENT_MODE),
			getDolGlobalString(LmdbVehicleConsumptionPayment::CONST_ACCOUNTING_ACCOUNT),
			getDolGlobalString(LmdbVehicleConsumptionPayment::CONST_SUBLEDGER_ACCOUNT),
			(int) $conf->entity
		) < 0) {
			setEventMessages($langs->trans($odConfigurationCheck->error ?: 'ConsumptionOdConfigurationIncomplete'), null, 'errors');
		}
	}
	print load_fiche_titre($title, '', 'gas-pump');
	print '<form class="lmdb-responsive-form" method="POST" enctype="multipart/form-data" action="'.$_SERVER['PHP_SELF'].'"><input type="hidden" name="token" value="'.newToken().'"><input type="hidden" name="action" value="'.($action === 'create' ? 'add' : 'update').'">';
	if ($id > 0) print '<input type="hidden" name="id" value="'.$id.'">';
	print '<div class="div-table-responsive-no-min"><table class="border centpercent tableforfield">';
	if ($financialLocked) {
		$lockedOptions = $nature === 'fuel' ? $fuelOptions : $additiveOptions;
		$selectedConsumableLabel = isset($lockedOptions[(int) $object->fk_consumable]) ? $lockedOptions[(int) $object->fk_consumable] : '';
		print '<tr><td class="titlefieldcreate">'.$langs->trans('ConsumptionDate').'</td><td>'.dol_print_date($object->reading_date, 'dayhour').'</td></tr>';
		print '<tr><td>'.$langs->trans('Vehicle').'</td><td>'.dol_escape_htmltag(isset($vehicleOptions[(int) $object->fk_vehicle]) ? $vehicleOptions[(int) $object->fk_vehicle] : '').'</td></tr>';
		print '<tr><td>'.$langs->trans('ConsumptionNature').'</td><td>'.$langs->trans($nature === 'fuel' ? 'FuelOrRecharge' : 'Additive').'</td></tr>';
		print '<tr><td>'.$langs->trans('Consumable').'</td><td>'.dol_escape_htmltag($selectedConsumableLabel).'</td></tr>';
	} else {
	...
	if ($canReadProject) {
		...
		if ($financialLocked) {
			...
			print !empty($object->fk_project) && $project->fetch((int) $object->fk_project) > 0 ? $project->getNomUrl(1) : '';
		} else {
			print $formProject->select_projects(-1, !empty($object->fk_project) ? (string) $object->fk_project : '', 'fk_project', 0, 0, 1, 1, 0, 0, 0, '', 1, 0, 'maxwidth500');
		}
		print '</td></tr>';
	}
	if ($action === 'create' && $odFeatureEnabled) {
		print '<tr id="payment_mode_row"><td class="fieldrequired">'.$langs->trans('PaymentMode').'</td><td>';
		print $form->select_types_paiements(getDolGlobalInt(LmdbVehicleConsumptionPayment::CONST_PAYMENT_MODE), 'payment_mode_id', 'DBIT', 0, 1, 1, 0, 1, 'minwidth300', 1);
		print '</td></tr>';
		print '<tr id="receipt_row"><td class="fieldrequired">'.$langs->trans('ConsumptionReceipt').'</td><td><input type="file" name="receipt" accept="application/pdf,image/jpeg,image/png"> <span class="opacitymedium">'.$langs->trans('ConsumptionReceiptFormats').'</span></td></tr>';
	}
	if (!$financialLocked || $nature === 'additive') print '<tr id="oil_reference_row"><td id="oil_reference_label">'.$langs->trans('OilReference').'</td><td><input class="flat minwidth300" name="oil_reference" maxlength="128" value="'.dol_escape_htmltag((string) $object->oil_reference).'"></td></tr>';
	...
	$projectLink = '';
	if ($canReadProject && !empty($object->fk_project)) {
		$project = new Project($db);
		$projectLink = $project->fetch((int) $object->fk_project) > 0 ? $project->getNomUrl(1) : '';
	}
	$percentage = $object->getCapacityPercentage();
	...
	if (!empty($object->fk_payment_various)) {
		$paymentLabel = $langs->trans('VariousPayment').' #'.((int) $object->fk_payment_various);
		$paymentOutput = dol_escape_htmltag($paymentLabel);
		if ((int) $object->entity === (int) $conf->entity && $user->hasRight('banque', 'lire')) {
			$paymentOutput = '<a href="'.dol_escape_htmltag($consumptionPayment->getPaymentUrl($object)).'">'.img_picto('', 'bank', 'class="pictofixedwidth"').dol_escape_htmltag($paymentLabel).'</a>';
		}
		print '<tr><td>'.$langs->trans('ConsumptionVariousPayment').'</td><td>'.$paymentOutput.'</td></tr>';
		$receiptPath = $consumptionPayment->getReceiptPath($object);
		print '<tr><td>'.$langs->trans('ConsumptionReceipt').'</td><td>';
		if ($receiptPath !== '') {
			print '<a href="'.dol_buildpath('/lmdbvehiclemanagement/consumption_receipt.php', 1).'?id='.$id.'">'.img_picto('', 'paperclip', 'class="pictofixedwidth"').dol_escape_htmltag(basename($receiptPath)).'</a>';
		} else {
	...
	if ($odLocked > 0) print '<div class="warning">'.$langs->trans('ConsumptionOdLocked').'</div>';
	...
	if ($permissionDelete && $odLocked <= 0) print dolGetButtonAction('', $langs->trans('Delete'), 'delete', $_SERVER['PHP_SELF'].'?id='.$id.'&action=delete&token='.newToken());
	print '</div><div class="fichecenter"><div class="fichehalfleft">';
	if ($permissionWrite && !empty($object->fk_payment_various) && $odLocked <= 0) {
		print '<form method="POST" enctype="multipart/form-data" action="'.dol_escape_htmltag($_SERVER['PHP_SELF']).'">';
		print '<input type="hidden" name="token" value="'.newToken().'"><input type="hidden" name="id" value="'.$id.'"><input type="hidden" name="action" value="replace_receipt">';
		print '<div class="div-table-responsive-no-min"><table class="noborder centpercent"><tr class="liste_titre"><th colspan="2">'.$langs->trans('ReplaceConsumptionReceipt').'</th></tr>';
		print '<tr class="oddeven"><td class="fieldrequired">'.$langs->trans('ConsumptionReceipt').'</td><td><input type="file" name="receipt" accept="application/pdf,image/jpeg,image/png"> <input type="submit" class="button" value="'.$langs->trans('Replace').'"></td></tr></table></div></form>';
	}
