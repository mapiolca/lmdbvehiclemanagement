...
$where = " WHERE a.elementtype = 'lmdbvehicle@lmdbvehiclemanagement' AND a.fk_element = ".((int) $id);
$where .= ' AND a.entity IN ('.getEntity('agenda').')';
if (!$canReadAllAgenda) {
	$where .= ' AND (a.fk_user_author = '.((int) $user->id);
	$where .= ' OR a.fk_user_action = '.((int) $user->id);
	$where .= ' OR EXISTS (SELECT 1 FROM '.MAIN_DB_PREFIX.'actioncomm_resources AS ar';
	$where .= " WHERE ar.fk_actioncomm = a.id AND ar.element_type = 'user' AND ar.fk_element = ".((int) $user->id).'))';
}
...
while ($rendered < min($num, $limit) && is_object($row = $db->fetch_object($resql))) {
	...
	if (!$canReadAllAgenda) {
		$agendaObject->userassigned[(int) $user->id] = array('id' => (int) $user->id, 'transparency' => 0);
	}
	$owner = new User($db);
