...
if ($action === 'create' || $action === 'edit') {
	...
	print '</table></div><div class="center"><button type="submit" class="button button-save">'.$langs->trans('Save').'</button> <button type="submit" class="button button-cancel" name="cancel" formnovalidate>'.$langs->trans('Cancel').'</button></div></form>';
	...
	if ($permissionValidate && (int) $object->status === LmdbVehicleRegulatoryControl::STATUS_CANCELLED) { print dolGetButtonAction('', $langs->trans('CreateReplacementControl'), 'default', $_SERVER['PHP_SELF'].'?action=create&previous_id='.$id.'&vehicle_id='.((int) $object->fk_vehicle).'&requirement_id='.((int) $object->fk_requirement)); print dolGetButtonAction('', $langs->trans('Archive'), 'default', $_SERVER['PHP_SELF'].'?id='.$id.'&action=archive&token='.newToken()); }
