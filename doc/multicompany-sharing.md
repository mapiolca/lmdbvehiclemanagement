...
4. Depuis l’entité propriétaire, un administrateur renseigne le partage. Sur un véhicule, le sélecteur apparaît uniquement à la fin des formulaires **Créer** et **Modifier**, avant **Enregistrer** ; les champs, capacités et partages sont enregistrés ensemble. Dans **Utilisation QUARTIX**, sélectionner la source puis utiliser **Gérer le partage des données QUARTIX** pour ouvrir sa modale dédiée. Pour les autres fiches, le réglage reste dans leur écran existant ; le lien globe des attestations, affectations et relevés permet de l’ouvrir. Les destinations proposées sont actives, accessibles à cet administrateur et configurées pour recevoir cette famille depuis le propriétaire.
...
Le sélecteur intégré nécessite le JavaScript natif Multicompany. Si son chargement échoue, enregistrer les champs du véhicule conserve les partages existants et l’enregistrement de la modale QUARTIX reste désactivé.
