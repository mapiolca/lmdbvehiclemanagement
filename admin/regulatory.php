...
if ($action === 'create_override') {
	$effectiveFrom = GETPOSTINT('override_effective_fromyear') > 0 ? dol_mktime(12, 0, 0, GETPOSTINT('override_effective_frommonth'), GETPOSTINT('override_effective_fromday'), GETPOSTINT('override_effective_fromyear')) : null;
	$effectiveTo = GETPOSTINT('override_effective_toyear') > 0 ? dol_mktime(12, 0, 0, GETPOSTINT('override_effective_tomonth'), GETPOSTINT('override_effective_today'), GETPOSTINT('override_effective_toyear')) : null;
	$numericValues = array();
...
if ($action === 'create_custom_rule') {
	$effectiveFrom = GETPOSTINT('custom_effective_fromyear') > 0 ? dol_mktime(12, 0, 0, GETPOSTINT('custom_effective_frommonth'), GETPOSTINT('custom_effective_fromday'), GETPOSTINT('custom_effective_fromyear')) : null;
	$effectiveTo = GETPOSTINT('custom_effective_toyear') > 0 ? dol_mktime(12, 0, 0, GETPOSTINT('custom_effective_tomonth'), GETPOSTINT('custom_effective_today'), GETPOSTINT('custom_effective_toyear')) : null;
	...
	$profileIds = GETPOSTISARRAY('custom_profile_ids') ? array_values(array_unique(array_map('intval', GETPOST('custom_profile_ids', 'array:int')))) : array();
	$values = array(
...
if ($action === 'save') {
	$horizons = array_values(array_unique(array_filter(array_map('intval', preg_split('/[^0-9]+/', GETPOST('reminder_horizons', 'alphanohtml'), -1, PREG_SPLIT_NO_EMPTY)), static function ($value) { return $value >= 0; })));
...
$form = new Form($db); $userOptions = array(); $resql = $db->query('SELECT rowid, firstname, lastname, login FROM '.MAIN_DB_PREFIX.'user WHERE entity IN (0,'.((int) $conf->entity).') AND statut = 1 ORDER BY lastname, firstname, login'); if ($resql) { while (is_object($row = $db->fetch_object($resql))) { $name = trim((string) $row->firstname.' '.(string) $row->lastname); $userOptions[(int) $row->rowid] = $name !== '' ? $name : (string) $row->login; } $db->free($resql); }
$groupOptions = array(); $resql = $db->query('SELECT rowid, nom FROM '.MAIN_DB_PREFIX.'usergroup WHERE entity IN (0,'.((int) $conf->entity).') ORDER BY nom'); if ($resql) { while (is_object($row = $db->fetch_object($resql))) $groupOptions[(int) $row->rowid] = (string) $row->nom; $db->free($resql); }
...
print '<tr class="oddeven"><td>'.$langs->trans('ReminderHorizonsDays').'</td><td><input class="flat minwidth300" name="reminder_horizons" value="'.dol_escape_htmltag(implode(', ', array_map('intval', $horizons))).'"></td></tr><tr class="oddeven"><td>'.$langs->trans('DueSoonDays').'</td><td><input class="flat width75" inputmode="numeric" name="due_soon_days" value="'.getDolGlobalInt('LMDBVEHICLEMANAGEMENT_CONTROL_DUE_SOON_DAYS', 90).'"></td></tr>';
print '<tr class="oddeven"><td>'.$langs->trans('RecipientUsers').'</td><td>'.$form->multiselectarray('recipient_users', $userOptions, array_map('intval', $selectedUsers), 0, 0, 'minwidth500').'</td></tr><tr class="oddeven"><td>'.$langs->trans('RecipientGroups').'</td><td>'.$form->multiselectarray('recipient_groups', $groupOptions, array_map('intval', $selectedGroups), 0, 0, 'minwidth500').'</td></tr><tr class="oddeven"><td>'.$langs->trans('EmailTemplate').'</td><td>'.$form->selectarray('reminder_template', $templateOptions, getDolGlobalInt('LMDBVEHICLEMANAGEMENT_REGULATORY_REMINDER_TEMPLATE'), 1, 0, 0, '', 1, 0, 0, '', 'minwidth500', 1).'</td></tr></table></div><div class="center"><button class="button button-save" type="submit">'.$langs->trans('Save').'</button></div></form>';
print load_fiche_titre($langs->trans('RegulatoryRuleCatalog'), '', 'book-open');
print '<div class="div-table-responsive-no-min"><table class="noborder centpercent"><tr class="liste_titre"><th>'.$langs->trans('RegulatoryRuleCode').'</th><th>'.$langs->trans('Label').'</th><th>'.$langs->trans('RuleOrigin').'</th><th>'.$langs->trans('RulePeriodicity').'</th><th>'.$langs->trans('RuleBlockingMode').'</th><th>'.$langs->trans('Status').'</th><th>'.$langs->trans('OverrideReason').'</th></tr>';
foreach ($ruleRows as $ruleRow) {
	...
	print '<tr class="oddeven"><td>'.dol_escape_htmltag((string) $ruleRow->code).'</td><td>'.$ruleLabel.'</td><td>'.$langs->trans(!empty($ruleRow->is_native) ? 'NativeRule' : 'EntityRuleOverride').(!empty($ruleRow->parent_label) ? ' — '.dol_escape_htmltag($langs->trans((string) $ruleRow->parent_label)) : '').'</td><td>'.dol_escape_htmltag(implode(' — ', $periodParts)).'</td><td>'.$langs->trans('BlockingMode'.ucfirst((string) $ruleRow->default_blocking_mode)).'</td><td>'.dolGetStatus($langs->trans(!empty($ruleRow->active) ? 'Enabled' : 'Disabled'), '', '', !empty($ruleRow->active) ? 'status4' : 'status0', 1).'</td><td>'.dol_escape_htmltag((string) $ruleRow->override_reason).'</td></tr>';
}
if (empty($ruleRows)) print '<tr class="oddeven"><td colspan="7"><span class="opacitymedium">'.$langs->trans('NoRecordFound').'</span></td></tr>';
print '</table></div>';
print load_fiche_titre($langs->trans('CreateRegulatoryRuleOverride'), '', 'pen');
print '<div class="info">'.$langs->trans('OverrideBlankValuesInherited').'</div>';
print '<form method="POST" action="'.$_SERVER['PHP_SELF'].'"><input type="hidden" name="token" value="'.newToken().'"><input type="hidden" name="action" value="create_override"><div class="div-table-responsive-no-min"><table class="border centpercent">';
print '<tr><td class="titlefieldcreate fieldrequired">'.$langs->trans('ParentRegulatoryRule').'</td><td>'.$form->selectarray('parent_rule_id', $nativeRuleOptions, GETPOSTINT('parent_rule_id'), 1, 0, 0, '', 1, 0, 0, '', 'minwidth500', 1).'</td></tr>';
print '<tr><td>'.$langs->trans('OverrideLabel').'</td><td><input class="flat minwidth500" name="override_label" value="'.dol_escape_htmltag(GETPOST('override_label', 'alphanohtml')).'"></td></tr>';
print '<tr><td>'.$langs->trans('InitialDelayMonths').'</td><td><input class="flat width75" inputmode="numeric" name="override_initial_delay_months" value="'.dol_escape_htmltag(GETPOST('override_initial_delay_months', 'alphanohtml')).'"> '.$langs->trans('Month').'</td></tr>';
print '<tr><td>'.$langs->trans('RecurrenceMonths').'</td><td><input class="flat width75" inputmode="numeric" name="override_recurrence_months" value="'.dol_escape_htmltag(GETPOST('override_recurrence_months', 'alphanohtml')).'"> '.$langs->trans('Month').'</td></tr>';
print '<tr><td>'.$langs->trans('RecurrenceDays').'</td><td><input class="flat width75" inputmode="numeric" name="override_recurrence_days" value="'.dol_escape_htmltag(GETPOST('override_recurrence_days', 'alphanohtml')).'"> '.$langs->trans('Days').'</td></tr>';
print '<tr><td>'.$langs->trans('RecheckDays').'</td><td><input class="flat width75" inputmode="numeric" name="override_recheck_days" value="'.dol_escape_htmltag(GETPOST('override_recheck_days', 'alphanohtml')).'"> '.$langs->trans('Days').'</td></tr>';
...
print '<tr><td>'.$langs->trans('RuleBlockingMode').'</td><td>'.$form->selectarray('override_blocking_mode', $blockingOptions, GETPOST('override_blocking_mode', 'aZ09'), 0, 0, 0, '', 1, 0, 0, '', 'minwidth300', 1).'</td></tr>';
print '<tr><td>'.$langs->trans('RuleEffectivePeriod').'</td><td>'.$form->selectDate(-1, 'override_effective_from', 0, 0, 1, '', 1, 1).' — '.$form->selectDate(-1, 'override_effective_to', 0, 0, 1, '', 1, 1).'</td></tr>';
print '<tr><td class="fieldrequired">'.$langs->trans('OverrideReason').'</td><td><textarea class="flat centpercent" name="override_reason" rows="3">'.dol_escape_htmltag(GETPOST('override_reason', 'restricthtml')).'</textarea></td></tr>';
print '</table></div><div class="center"><button class="button button-save" type="submit">'.$langs->trans('CreateRegulatoryRuleOverride').'</button></div></form>';
print load_fiche_titre($langs->trans('CreateCustomRegulatoryRule'), '', 'plus-circle');
print '<form method="POST" action="'.$_SERVER['PHP_SELF'].'"><input type="hidden" name="token" value="'.newToken().'"><input type="hidden" name="action" value="create_custom_rule"><div class="div-table-responsive-no-min"><table class="border centpercent">';
print '<tr><td class="titlefieldcreate fieldrequired">'.$langs->trans('Label').'</td><td><input class="flat minwidth500" name="custom_label" value="'.dol_escape_htmltag(GETPOST('custom_label', 'alphanohtml')).'"></td></tr>';
print '<tr><td>'.$langs->trans('Description').'</td><td><textarea class="flat centpercent" name="custom_description" rows="2">'.dol_escape_htmltag(GETPOST('custom_description', 'restricthtml')).'</textarea></td></tr>';
$selectedCustomProfiles = GETPOSTISARRAY('custom_profile_ids') ? array_map('intval', GETPOST('custom_profile_ids', 'array:int')) : array();
print '<tr><td class="fieldrequired">'.$langs->trans('RegulatoryProfiles').'</td><td>'.$form->multiselectarray('custom_profile_ids', $regulatoryProfileOptions, $selectedCustomProfiles, 0, 0, 'minwidth500').'</td></tr>';
...
print '<tr><td class="fieldrequired">'.$langs->trans('RuleCalculator').'</td><td>'.$form->selectarray('custom_calculator_code', $calculatorOptions, GETPOST('custom_calculator_code', 'aZ09') ?: 'periodic_months', 0, 0, 0, '', 1, 0, 0, '', 'minwidth300', 1).'</td></tr>';
print '<tr><td>'.$langs->trans('InitialDelayMonths').'</td><td><input class="flat width75" inputmode="numeric" name="custom_initial_delay_months" value="'.dol_escape_htmltag(GETPOST('custom_initial_delay_months', 'alphanohtml')).'"> '.$langs->trans('Month').'</td></tr>';
print '<tr><td>'.$langs->trans('RecurrenceMonths').'</td><td><input class="flat width75" inputmode="numeric" name="custom_recurrence_months" value="'.dol_escape_htmltag(GETPOST('custom_recurrence_months', 'alphanohtml')).'"> '.$langs->trans('Month').'</td></tr>';
print '<tr><td>'.$langs->trans('RecurrenceDays').'</td><td><input class="flat width75" inputmode="numeric" name="custom_recurrence_days" value="'.dol_escape_htmltag(GETPOST('custom_recurrence_days', 'alphanohtml')).'"> '.$langs->trans('Days').'</td></tr>';
print '<tr><td>'.$langs->trans('RecheckDays').'</td><td><input class="flat width75" inputmode="numeric" name="custom_recheck_days" value="'.dol_escape_htmltag(GETPOST('custom_recheck_days', 'alphanohtml')).'"> '.$langs->trans('Days').'</td></tr>';
print '<tr><td>'.$langs->trans('RuleBlockingMode').'</td><td>'.$form->selectarray('custom_blocking_mode', $blockingOptions, GETPOST('custom_blocking_mode', 'aZ09') ?: 'none', 0, 0, 0, '', 1, 0, 0, '', 'minwidth300', 1).'</td></tr>';
print '<tr><td>'.$langs->trans('RuleEffectivePeriod').'</td><td>'.$form->selectDate(-1, 'custom_effective_from', 0, 0, 1, '', 1, 1).' — '.$form->selectDate(-1, 'custom_effective_to', 0, 0, 1, '', 1, 1).'</td></tr>';
print '<tr><td class="fieldrequired">'.$langs->trans('CustomizationReason').'</td><td><textarea class="flat centpercent" name="custom_reason" rows="3">'.dol_escape_htmltag(GETPOST('custom_reason', 'restricthtml')).'</textarea></td></tr>';
print '</table></div><div class="center"><button class="button button-save" type="submit">'.$langs->trans('CreateCustomRegulatoryRule').'</button></div></form>';
