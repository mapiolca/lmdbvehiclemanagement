...
trait LmdbVehicleSupplierInvoiceHooks
{
	...
	public function doActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($object instanceof LmdbVehicle && $action === 'builddoc') {
			...
			if (!(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'read')) || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'lmdbvehicle', 'write')) || !$user->hasRight('fournisseur', 'facture', 'lire') || !empty($user->socid)) accessforbidden();
			$langs->load('main');
		...
		try {
			if ($object instanceof FactureFournisseur && $action === 'add' && ($type !== '' || $sourceId > 0)) {
				...
				if (!LmdbVehicleManagementCompatibility::isFeatureAvailable('supplier_invoice_links') || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'read')) || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', $right, 'write')) || !$user->hasRight('fournisseur', 'facture', 'lire') || !$user->hasRight('fournisseur', 'facture', 'creer') || !empty($user->socid)) throw new RuntimeException('NotEnoughPermissions');
	...
	public function addMoreActionsButtons($parameters, &$object, &$action, $hookmanager)
	{
		...
		if (!LmdbVehicleManagementCompatibility::isFeatureAvailable('supplier_invoice_links') || (int) $object->entity !== (int) $conf->entity || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'read')) || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', $right, 'write')) || !$user->hasRight('fournisseur', 'facture', 'lire') || !$user->hasRight('fournisseur', 'facture', 'creer') || !empty($user->socid)) return 0;
	...
	public function showLinkToObjectBlock($parameters, &$object, &$action, $hookmanager)
	{
		global $conf, $user, $langs;
		$this->results = array();
		if ($object->element !== 'invoice_supplier' || !LmdbVehicleManagementCompatibility::isFeatureAvailable('supplier_invoice_links') || (int) $object->entity !== (int) $conf->entity || !(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'read')) || !$user->hasRight('fournisseur', 'facture', 'lire') || !$user->hasRight('fournisseur', 'facture', 'creer') || !empty($user->socid)) return 0;
		...
		foreach (array('event' => array('vehicle_event', 'lmdbvehicleevent', 'VehicleEvent'), 'regulatorycontrol' => array('regulatory_control', 'lmdbvehicleregulatorycontrol', 'RegulatoryControl')) as $right => $definition) {
			if (!(isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', $right, 'write'))) continue;
			$this->results['lmdbvehiclemanagement_'.$definition[1]] = array('enabled' => true, 'perms' => true, 'label' => $definition[2],
				'sql' => 'SELECT t.rowid, t.ref, 0 AS socid, v.label AS name, 0 AS client, 0 AS fournisseur, NULL AS ref_supplier, NULL AS total_ht FROM '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_'.$definition[0].' t INNER JOIN '.MAIN_DB_PREFIX.'lmdbvehiclemanagement_vehicle v ON v.rowid = t.fk_vehicle AND '.LmdbVehicleSharing::sql($this->db, 'lmdbvehicle', 'v').' WHERE t.entity = '.((int) $conf->entity).' AND '.LmdbVehicleSharing::sql($this->db, $definition[1]).' ORDER BY t.ref');
		}
		return 0;
	}
