...
$langs->loadLangs(array('other', 'mails', 'lmdbvehiclemanagement@lmdbvehiclemanagement'));
$id = GETPOSTINT('id');
...
$permissiontoadd = (isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'consumption', 'write')) ? 1 : 0;
$permissiontodelete = (isModEnabled('lmdbvehiclemanagement') && empty($user->socid) && $user->hasRight('lmdbvehiclemanagement', 'consumption', 'delete')) ? 1 : 0;
$upload_dirold = '';
...
$hookmanager->initHooks(array('lmdbvehicleconsumptiondocument', 'globalcard'));
...
print dol_get_fiche_head($head, 'documents', $langs->trans('ConsumptionEntry'), -1, $object->picto);
...
print '<tr><td class="titlefield">'.$langs->trans('NbOfAttachedFiles').'</td><td>'.count($filearray).'</td></tr><tr><td>'.$langs->trans('TotalSizeOfAttachedFiles').'</td><td>'.$totalsize.' '.$langs->trans('bytes').'</td></tr></table></div>';
